...
func UseEZFFormula(hTdb C.THANDLE){
	...
	fmt.Printf("Add Formula Result:%s",Char2byte(uintptr(unsafe.Pointer(&addRes.chInfo)),unsafe.Sizeof(addRes.chInfo[0]),len(addRes.chInfo)))
	...
	cpy("KDJ", tmpPtr_reqCalc, sizeOf_reqCalc, len(reqCalc.chFormulaName))
	...
	cpy("N=9,M1=3,M2=3", tmpPtr_chParam, sizeOf_chParam, len(reqCalc.chParam))
	...
	cpy("000001.SZ", tmpPtr_chCode, sizeOf_chCode, len(reqCalc.chCode))
	...
	cpy("SZ-2-0", tmpPtr_chMarketKey, sizeOf_chMarketKey, len(reqCalc.chCode))
